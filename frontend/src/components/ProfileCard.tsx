...
const ProfileCard: FC<ProfileCardProps> = ({ id, leader }) : JSX.Element => {
  return (
    <Box className='w-36 h-40 relative rounded-lg box-border overflow-hidden shadow-sm shadow-slate-500 flex justify-center'>
      ...
      <Box className='h-1/3 hover:h-3/5 transition-all ease-in-out w-full bg-primary'>
      </Box>
      {/* <Box className='h-2/3'>
      </Box> */}
      <Box className='absolute bottom-5 flex flex-wrap justify-center'>
        <p className='text-md w-full text-center mt-8 h-fit hover:underline' style={{ fontFamily: 'MetropolisSemiBold' }}>Aidan Wibrata</p>
